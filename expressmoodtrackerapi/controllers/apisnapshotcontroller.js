...
exports.getEditSingleSnapshotv2 = (req, res) => {
  ...
  conn.query(selectUserSnapshotsSQL, [id, user_ID], (err, snapshot) => {
    if (err) {
      ...
      // Corrected from 'rows' to 'snapshot'
      conn.query(
        selectChosenTriggersSQL,
        [id],
        (triggerErr, selectedTriggers) => {
          if (triggerErr) {
            res.status(500);
            res.json({
              status: "failure",
            ...
            conn.query(selectTriggersSQL, (triggerErr, triggers) => {
              // Removed [id] as parameter
              if (triggerErr) {
                res.status(500);
                res.json({
                  status: "failure",
        ...
        if (err) {
          return conn.rollback(() => {
            res
              .status(500)
              .json({
                error: "Error inserting snapshot",
                message: err.message,
              });
          });
        }
            ...
            if (err) {
              return conn.rollback(() => {
                res
                  .status(500)
                  .json({
                    error: "Error searching for trigger names",
                    message: err.message,
                  });
              });
            }
...
exports.updateSnapshot = (req, res) => {
  ...
  conn.beginTransaction((err) => {
    ...
    conn.query(
      ...
      (err, updateSnapshotResult) => {
        if (err) {
          return conn.rollback(() => {
            res
              .status(500)
              .json({ error: "Error updating snapshot", message: err.message });
          });
        }
        ...
        if (updateSnapshotResult.affectedRows === 0) {
          return conn.rollback(() => {
            res
              .status(404)
              .json({
                error: "Snapshot not found - check user_ID and snapshot_ID",
              });
          });
        }
        console.log("Update snapshot result:", updateSnapshotResult);
        conn.query(
          deleteSnapshotTriggersSQL,
          [snapshot_ID],
          (err, deleteTriggersResult) => {
            if (err) {
              return conn.rollback(() => {
                res
                  .status(500)
                  .json({
                    error: "Error deleting triggers",
                    message: err.message,
                  });
              });
            }
